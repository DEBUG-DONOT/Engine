...
void main()
{
    ...
    for(int i=0;i<kernelSize;i++)
    {
        ...
        float depthValue= -texture(texture3, offset.xy).a;//??????? 为什么取负号？
        float rangeCheck = smoothstep(0.0, 1.0, radius / abs(fragPosition.z - depthValue));
        occlusion+= depthValue>curr_sample.z? 1.0:0.0;
    }
